...
export default function IndustryPage() {
  const { features, error, savedReloadKey, industry } = useWorkspace();
    ...
    <div className="lg:grid lg:grid-cols-[22rem_minmax(0,1fr)]">
      <aside className="border-b border-line bg-surface lg:sticky lg:top-14 lg:h-[calc(100vh-3.5rem)] lg:overflow-hidden lg:border-b-0 lg:border-r">
        <IndustryRail
          ...
          onTitleChange={industry.setTitle}
          prompt={industry.prompt}
          onPromptChange={industry.setPrompt}
          files={industry.files}
          onFilesChange={industry.setFiles}
          onRun={industry.run}
          busy={industry.uploading || industry.runs.some((r) => r.status === "running")}
          disabled={claudeOff}
        />
      </aside>
